package com.udi.app.framework.utilities.map;
...
import com.udi.app.framework.listeners.AddressListener;
import com.udi.app.framework.listeners.DirectionsListener;
import com.udi.app.framework.listeners.DistanceListener;
import com.udi.app.framework.listeners.ErrorListener;
import com.udi.app.framework.listeners.GooglePlacesListener;
import com.udi.app.framework.listeners.TurnByTurnListener;
import com.udi.app.framework.net.JsonRequest;
import com.udi.app.framework.net.JsonResponseListener;
import com.udi.app.framework.net.VolleyClient;
import com.udi.app.framework.utilities.FrameworkUtils;
import com.udi.app.framework.utilities.map.model.TurnByTurnModel;
import com.udi.app.framework.utilities.map.polyline.RainbowGroundOverlayUtils;
import com.udi.app.framework.utilities.map.polyline.RainbowPoint;
import com.udi.app.framework.utilities.map.polyline.RainbowPolyline;
import com.udi.app.framework.utilities.map.polyline.RainbowPolylineOptions;
...
public class GoogleServiceUtility {
    ...
    private static ArrayList<RainbowPoint> alRainbowPoints;
    private static RainbowPolyline mRainbowPolyline;
    private static RainbowGroundOverlayUtils mRainbowGroundOverlay;
     ...
     * @return
     */
    public static void createRainbowPolyline(Context context, RainbowGroundOverlayUtils overlayUtils,
                                             LatLng origin, LatLng dest, int colorStartEnd, int colorMid) {
        // confirm that origin and dest LatLng is not null, otherwise throw exception
        if (FrameworkUtils.checkIfNull(origin) || FrameworkUtils.checkIfNull(dest)) {
            throw new IllegalArgumentException("Origin and Dest LatLng cannot be null");
        }

        // retrieve rainbow points
        alRainbowPoints = createRainbowPoints(context, origin, dest, colorStartEnd, colorMid);
        mRainbowGroundOverlay = overlayUtils;

        // create rainbow polyline
        RainbowPolylineOptions options = new RainbowPolylineOptions(null)
                .zIndex(0).strokeWidth(ARC_STROKE_WIDTH).strokeColor(Color.BLACK)
                .linearGradient(true).add(alRainbowPoints);

        // create polyline
        mRainbowPolyline = options.build();
        mRainbowGroundOverlay.addPolylineShape(mRainbowPolyline);
    }

    /**
     * Method is used to generate points between origin and end and reposition as Bezier curve
     *
     * @param context
     * @param origin
     * @param dest
     ...
     * @return
     */
    private static ArrayList<RainbowPoint> createRainbowPoints(Context context, LatLng origin, LatLng dest,
                                                               int colorStartEnd, int colorMid) {
        // create new list of rainbow points
        ArrayList<RainbowPoint> alPoints = new ArrayList<>();
        ...
        // midpoint position
        LatLng midpoint = SphericalUtil.computeOffset(origin, (distance * 0.5), heading);

        // calculate arc threshold
        double arcThreshold = computeArcThreshold(distance);

        // calculate position of the circle center
        double x = (1 - arcThreshold * arcThreshold) * distance * 0.5 / (2 * arcThreshold);
        double r = (1 + arcThreshold * arcThreshold) * distance * 0.5 / (2 * arcThreshold);

        // calculate offset
        LatLng offset;
        if (heading > 0) {
            offset = SphericalUtil.computeOffset(midpoint, x, heading + 90.0);
        } else {
            offset = SphericalUtil.computeOffset(midpoint, x, heading - 90.0);
        }

        // calculate heading between circle center and two points
        double headingA = SphericalUtil.computeHeading(offset, origin);
        double headingB = SphericalUtil.computeHeading(offset, dest);

        // calculate number of points
        int numBezierCurvePoints = computeNumberOfCurvePoints(distance);

        // calculate positions of points on circle border and add them to polyline options
        double step = (headingB - headingA) / numBezierCurvePoints;

        for (int i = 0; i < numBezierCurvePoints; i++) {
            // add origin point
            if (i == 0) {
                alPoints.add(new RainbowPoint(new LatLng(origin.latitude, origin.longitude))
                        .color(ContextCompat.getColor(context, colorStartEnd)));
            }

            LatLng pi = SphericalUtil.computeOffset(offset, r, headingA + i * step);
            if (i > (numBezierCurvePoints) * ARC_COLOR_PERCENTAGE ||
                    i < numBezierCurvePoints - (numBezierCurvePoints * ARC_COLOR_PERCENTAGE)) {
                alPoints.add(new RainbowPoint(new LatLng(pi.latitude, pi.longitude))
                        .color(ContextCompat.getColor(context, colorStartEnd)));
            } else {
                alPoints.add(new RainbowPoint(new LatLng(pi.latitude, pi.longitude))
            ...
            if (i == (numBezierCurvePoints - 1)) {
                alPoints.add(new RainbowPoint(new LatLng(dest.latitude, dest.longitude))
                        .color(ContextCompat.getColor(context, colorStartEnd)));
            }
        }
        return alPoints;
     ...
     * @return
     */
    public static boolean isRainbowPolylineVisible() {
        if (!FrameworkUtils.checkIfNull(mRainbowPolyline) && !FrameworkUtils.checkIfNull(alRainbowPoints)
                && alRainbowPoints.size() > 0 && !alRainbowPoints.isEmpty()) {
            return true;
        }
        return false;
    }
     ...
     */
    public static void clearRainbowPolyline() {
        if (!FrameworkUtils.checkIfNull(mRainbowPolyline) && !FrameworkUtils.checkIfNull(alRainbowPoints)
                && alRainbowPoints.size() > 0 && !alRainbowPoints.isEmpty()) {
            mRainbowGroundOverlay.removePolylineShape(mRainbowPolyline);
            mRainbowGroundOverlay.refresh();

            // clear/null objects
            alRainbowPoints.clear();
            alRainbowPoints = null;
            mRainbowGroundOverlay = null;
            mRainbowPolyline = null;
        }
    }
